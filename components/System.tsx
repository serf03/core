...
//Module Alert
import type { AccessUser, Client, GarmentType, Invoice, Product, ProductionRecord, User } from "../lib/types"
...
export function System() {
  ...
  const [newGarmentType, setNewGarmentType] = useState<Omit<GarmentType, "id">>({
    name: "",
    basePrice: 0,
    description: "",
    category: "",
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      if (user) {
        const userData = await firebaseServices.getUserById(user.uid)
        setCurrentUser(userData)
        if(dailyProduction) console.log();
        ;
      }
    }
    fetchCurrentUser()
  }, [user,dailyProduction])
  ...
  const filterClients = () => {
    return clients.filter((client) =>
      clientFilterType === "cedula" ? client.cedula.includes(clientFilter) : client.phone.includes(clientFilter),
    )
  }
  ...
  const navItems: { name: string; icon: JSX.Element; id: AccessUser }[] = [
  ...
  {navItems
    .filter(
      (item) =>
        ...
        currentUser?.accessibleViews.includes(item.id)
    )
    .map((item) => (
      <Button
        key={item.id}
            ...
            >
              <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                <Suspense fallback={<div>Cargando...</div>}>
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("reports")) &&
                    activeTab === "reports" && (
                      <TabsContent value="reports">
                        <TabsReport invoices={invoices} client={clients} />
                      </TabsContent>
                    )}
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("billing")) &&
                    activeTab === "billing" && (
                  ...
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("users")) &&
                    activeTab === "users" && (
                  ...
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("clients")) &&
                    activeTab === "clients" && (
                  ...
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("products")) &&
                    activeTab === "products" && (
                  ...
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("garmentTypes")) &&
                                  ...
                                  <TableRow key={garmentType.id}>
                                    <TableCell>
                                      <div className="flex items-center space-x-2">
                                        <Avatar className={`h-10 w-10 ${getAvatarColor(garmentType.name)}`}>
                                          <AvatarImage
                                            src={`https://api.dicebear.com/6.x/initials/svg?seed=${garmentType.name}`}
                                          />
                                    ...
                                    <TableCell>
                                      <div className="flex space-x-2">
                                        ...
                                        <Button
                                          variant="destructive"
                                          onClick={() => handleDeleteGarmentType(garmentType?.id)}
                                        >
                  ...
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("cierre")) &&
                    activeTab === "cierre" && (
                      <TabsContent value="cierre">
                        <TabCierreCaja />
                      </TabsContent>
                    )}
                  {(currentUser?.role === "Administrador" || currentUser?.accessibleViews.includes("expenses")) &&
                    activeTab === "expenses" && (
