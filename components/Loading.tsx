...
import { OrbitControls, Ring, Sphere, useTexture } from '@react-three/drei'
...
const PlanetRing: React.FC<RingProps> = ({ radius, tubeRadius, color, rotation }) => {
  const ringRef = useRef<THREE.Mesh>(null!)

  useFrame((state) => {
    const t = state.clock.getElapsedTime()
    ringRef.current.rotation.x = Math.sin(t * 1.5) * 0.5 + rotation[0]
    ringRef.current.rotation.y = Math.cos(t * 1.7) * 0.5 + rotation[1]
    ringRef.current.rotation.z += 0.01
  })

  return (
    <Ring ref={ringRef} args={[radius, radius + tubeRadius, 64]} rotation={rotation}>
      <meshPhongMaterial color={color} side={THREE.DoubleSide} transparent opacity={0.7} />
    </Ring>
  )
}
...
const Planet: React.FC = () => {
  ...
  const texture = useTexture(`data:image/svg+xml;base64,${btoa(svgTexture)}`);
  ...
  useFrame((state) => {
    ...
    planetRef.current.position.y = Math.sin(t * 0.2) * 0.02
  })

  return (
    <group>
      <Sphere ref={planetRef} args={[1, 64, 64]} material={planetMaterial} />
      <PlanetRing radius={1.3} tubeRadius={0.02} color="#e74c3c" rotation={[Math.PI / 2, 0, 0]} />
      <PlanetRing radius={1.5} tubeRadius={0.03} color="#f39c12" rotation={[Math.PI / 4, Math.PI / 6, 0]} />
      <PlanetRing radius={1.7} tubeRadius={0.025} color="#2ecc71" rotation={[-Math.PI / 3, -Math.PI / 6, 0]} />
    </group>
  )
}

interface CinematicPlanetLoadingProps {
  size?: number
}

const CinematicPlanetLoading: React.FC<CinematicPlanetLoadingProps> = ({ size = 300 }) => {
  return (
    <div className="fixed inset-0 flex items-center justify-center pointer-events-none" aria-live="polite" aria-busy="true">
...
export default CinematicPlanetLoading